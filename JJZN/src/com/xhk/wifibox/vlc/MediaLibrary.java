...
public class MediaLibrary {
	...
	public void loadMediaItems(Context context, long lengthScope) {
		mMinLength = lengthScope;
	...
	// public ArrayList<Media> getAudioItems(String name, String name2, int
	// mode) {
	// ArrayList<Media> audioItems = new ArrayList<Media>();
	// mItemListLock.readLock().lock();
	// for (int i = 0; i < mItemList.size(); i++) {
	...
	private class GetMediaItemsRunnable implements Runnable {
		...
		public void run() {
			...
			intent.setAction(Contants.BROADCAST_SCAN_START);
			mContext.sendBroadcast(intent);

			LibVLC libVlcInstance;
			try {
				libVlcInstance = Util.getLibVlcInstance(mContext);
			...
			try {
				...
				int total=mediaToScan.size();
				intent.setAction(Contants.BROADCAST_SCAN_TOTAL);
				intent.putExtra(Contants.EXTRA_SCAN_TOTAL, total);
				mContext.sendBroadcast(intent);
				
				// Process the stacked items
				for (File file : mediaToScan) {
					intent.setAction(Contants.BROADCAST_SCAN_COUNT);
					intent.putExtra(Contants.EXTRA_SCAN_COUNT, count++);
					mContext.sendBroadcast(intent);
					...
					if (existingMedias.containsKey(fileURI)) {
						...
						if ((mMinLength >= 0l && m.getLength() < mMinLength)
								|| (m.getType() != Media.TYPE_AUDIO)) {
							continue;
						}
						Log.i(TAG, ">>>>>" + m);
						mItemList.add(m);
						...
						MediaDatabase db = MediaDatabase
								.getInstance(XHKApplication.getAppContext());
						db.addMedia(m);
						mItemListLock.writeLock().unlock();
					}
				...
				if (mRestart) {
					...
					intent.setAction(Contants.BROADCAST_SCAN_FINISH);
					intent.putExtra(Contants.EXTRA_SCAN_COUNT, count++);
					mContext.sendBroadcast(intent);
